...
@Injectable()
export class JwtAllow3FirstStrategy extends PassportStrategy(Strategy, 'jwt-allow-3-first') {
    ...
    async validate(payload: any) {
        if (payload.type === 'temp_user' && payload.messageCount < 3) {
            const temp_user: TempUser = { messageCount: payload.messageCount, firstCreatedAt: payload.firstCreatedAt, type: payload.type };
            return temp_user;
        } else {
    ...
    private static extractTempUserJWT(req: RequestType): string | null {
        if (
            req?.cookies?.token?.temp_user_access_token?.length > 0
        ) {
